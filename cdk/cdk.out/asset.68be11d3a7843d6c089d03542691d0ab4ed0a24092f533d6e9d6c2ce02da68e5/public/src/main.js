...
// Wrap fetch so every /api/* request automatically carries the Cognito JWT
...
// Connect to the server. autoConnect is off so we can wait for the Cognito JWT
// before opening the socket; the connect happens at the end of DOMContentLoaded.
const socket = io({
    autoConnect: false,
    auth: (cb) => cb({ token: window.__authToken?.() }),
});
...
let config = {
    ...
    pushToTalk: false,         // when true, mic only streams while the button is held
};

// Per-CEFR-level response-length caps. Shorter responses = less Nova Sonic output cost.
// Applied automatically when the persona changes; user can override with the maxTokens slider.
...
function updateConfigFromSelect(selectId, value) {
    switch (selectId) {
        ...
        case 'prompt-preset':
            ...
            // Auto-adjust response length cap to match the level — A1/A2 stay short.
            const suggestedMax = MAX_TOKENS_BY_LEVEL[value];
            if (suggestedMax && config.maxTokens !== suggestedMax) {
                config.maxTokens = suggestedMax;
                if (maxTokensSlider) maxTokensSlider.value = suggestedMax;
                if (maxTokensValue) maxTokensValue.value = suggestedMax;
                if (typeof updateSliderTrack === 'function' && maxTokensSlider) updateSliderTrack(maxTokensSlider);
            }
            break;
        case 'scenario':
...
async function initSettings() {
    ...
    const pttChk = document.getElementById('push-to-talk');
    if (pttChk) {
        pttChk.checked = !!config.pushToTalk;
        pttChk.addEventListener('change', e => {
            config.pushToTalk = e.target.checked;
            saveConfig();
            applyPushToTalkUI();
        });
    }
...
async function startStreaming() {
    ...
    try {
        ...
        voiceHint.textContent = config.pushToTalk ? 'Release to stop' : 'Tap to stop';
...
function stopStreaming() {
    ...
    voiceHint.textContent = config.pushToTalk ? 'Hold to talk (or hold space)' : 'Tap to start conversation';
...
// ─── Server-emitted cost-control events ────────────────────────────────────
socket.on('usageUpdate', (data) => {
    ...
    const remainingMin = Math.max(0, Math.ceil((data.remainingSeconds || 0) / 60));
    const usedSec = Math.round(data.usedSeconds || 0);
    const limitSec = data.limitSeconds || 600;
    const pct = Math.min(100, (usedSec / limitSec) * 100);
    text.textContent = `Practice time left today: ${remainingMin} min`;
    fill.style.width = `${pct}%`;
    fill.classList.toggle('warning', pct >= 75 && pct < 100);
    fill.classList.toggle('danger', pct >= 90);
});

socket.on('idleTimeout', (data) => {
    if (isStreaming) stopStreaming();
    const div = document.createElement('div');
    div.className = 'message system';
    div.textContent = data?.reason || 'Session paused after 90 seconds of silence.';
    chatContainer.appendChild(div);
    scrollToBottom();
});

socket.on('replacedByNewSession', () => {
    if (isStreaming) stopStreaming();
    const div = document.createElement('div');
    div.className = 'message system';
    div.textContent = 'Another tab took over this session.';
    chatContainer.appendChild(div);
    scrollToBottom();
});

socket.on('dailyLimitReached', (data) => {
    if (isStreaming) stopStreaming();
    showDailyLimitModal(data?.message || "You've reached today's 10-minute practice limit. Come back tomorrow!");
});

function showDailyLimitModal(message) {
    if (document.getElementById('daily-limit-modal')) return;
    const overlay = document.createElement('div');
    overlay.id = 'daily-limit-modal';
    overlay.className = 'daily-limit-overlay';
    const card = document.createElement('div');
    ...
    ok.className = 'auth-submit';
    ok.textContent = 'OK, see you tomorrow';
...
// Voice button handler — two modes:
//   - Default (tap-to-toggle): tap starts, tap again stops.
//   - Push-to-talk: hold the button (or spacebar) to talk; release to stop.
// Push-to-talk dramatically cuts Nova Sonic input-audio cost because we don't stream during silences.
let pttHoldActive = false;
const pttHint = document.querySelector('.voice-hint');
const buttonWrapper = voiceBtn.parentElement;

function applyPushToTalkUI() {
    if (config.pushToTalk) {
        document.body.classList.add('push-to-talk-mode');
        if (pttHint) pttHint.textContent = isStreaming ? 'Release to stop' : 'Hold to talk (or hold space)';
    } else {
        document.body.classList.remove('push-to-talk-mode');
        if (pttHint) pttHint.textContent = isStreaming ? 'Tap to stop' : 'Tap to start conversation';
    }
}

voiceBtn.addEventListener('click', (e) => {
    if (config.pushToTalk) return;   // push-to-talk uses pointerdown/up below, not click
    if (isStreaming) stopStreaming();
    else startStreaming();
});

// ── Push-to-talk: pointer events cover mouse + touch + pen uniformly ──
voiceBtn.addEventListener('pointerdown', async (e) => {
    if (!config.pushToTalk || pttHoldActive) return;
    e.preventDefault();
    pttHoldActive = true;
    voiceBtn.classList.add('ptt-pressed');
    if (pttHint) pttHint.textContent = 'Listening — release when done';
...
function pttRelease() {
    ...
    pttHoldActive = false;
    voiceBtn.classList.remove('ptt-pressed');
    if (config.pushToTalk && isStreaming) {
        stopStreaming();
    }
    applyPushToTalkUI();
}

voiceBtn.addEventListener('pointerup', pttRelease);
voiceBtn.addEventListener('pointerleave', pttRelease);
voiceBtn.addEventListener('pointercancel', pttRelease);
...
document.addEventListener('keydown', async (e) => {
    ...
    if (!pttHoldActive) {
        pttHoldActive = true;
        voiceBtn.classList.add('ptt-pressed');
        if (pttHint) pttHint.textContent = 'Listening — release space when done';
        if (!isStreaming) await startStreaming();
    }
});
...
document.addEventListener('DOMContentLoaded', async () => {
    initTheme();
    // Block UI until user signs in (or auth is disabled). After this returns the JWT is in memory.
    await ensureAuthenticated();
    const signOutBtn = document.getElementById('auth-signout');
    if (signOutBtn) {
        signOutBtn.classList.remove('hidden');
        signOutBtn.addEventListener('click', () => window.__signOut?.());
    }
    await initSettings();
    initFeedbackPanel();
    initWaveformCanvas();
    // Now that we have a token, allow socket.io to connect — it was constructed in deferred mode
